...
runRandom
  :: forall q r a. R.RandomGen q => q -> Eff (Random ': r) a -> Eff r (q, a)
runRandom q = runState q
  . reinterpret (\case Random    -> do
                         ~(a, q') <- gets @q R.random
                         put q'
                         pure a
                       RandomR r -> do
...
runRandomIO :: (LastMember m r, MonadIO m) => Eff (Random ': r) a -> Eff r a
runRandomIO m = do
  q <- sendM $ liftIO $ R.newStdGen
  snd <$> runRandom q m
